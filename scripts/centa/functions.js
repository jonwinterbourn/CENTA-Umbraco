...
function showTweets(data, noOfTweets) {
	...
	var html = ['<ul id="twitter-updates-slider" style="width: auto; position: relative;">'];
	var maxNo = 0;
	...
	for (var i = 0; i < maxNo; i++) {
		...
		html.push('<li><div class="twitter-text"><p>',title,'</p>');
		//html.push(title, '</a></span></br><span class="tweet-hours">', timeAgo(date, "twitter"), '</span></div></div></li>');
		html.push('<a class="link" href="',tweet_url,'">',tweet_url,'</a><strong class="time">',timeAgo(date, "twitter"),'</strong></div></li>');
	}
	html.push('</ul>');
...
function fetchTweets(twitterAcc, noOfTweets) {
	...
	$.ajax({
            ...
            dataType:'jsonp',
			beforeSend: function() {
    			$('#twitter_feed').html("<img class='loader' src='/media/1140/loader2.gif' />");
  			},
...
function fetchTimeline(facebookAcc, noOfPosts) {
	...
	$.ajax({
            ...
            /*dataType:'jsonp',*/
            beforeSend: function() {
    			$('#facebook_timeline').html("<img class='loader' src='/media/1140/loader2.gif' />");
  			},
			success: function(data){
